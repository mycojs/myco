...
impl Op {
  ...
  fn gen(self) -> TokenStream2 {
    ...
    // Residual code from fast_call.rs
    let impl_and_fn = TokenStream2::new();
    ...
    let (v8_body, arg_count) = if is_async {
      ...
      codegen_v8_sync(&core, &item, attrs)
    };
...
fn codegen_v8_async(
  ...
  let wrapper = match (asyncness, is_result(&f.sig.output)) {
    ...
    (true, false) => {
      quote! {
        let fut = #core::_ops::map_async_op2(Self::call::<#type_params>(#args_head #args_tail));
        let maybe_response = #core::_ops::queue_async_op(
          ctx,
          scope,
          #deferred,
