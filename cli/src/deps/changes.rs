...
fn apply_deps_changes<T: AsRef<str>>(changes: &Vec<DepsChange>, toml: T) -> Result<String, MycoError> {
    let mut doc = toml.as_ref().parse::<Document>()
        .map_err(|e| MycoError::Internal { message: format!("Invalid TOML: {}", e) })?;
...
pub fn write_new_package_version(version: &PackageVersion, path: &PathBuf) -> Result<(), MycoError> {
    let contents = std::fs::read_to_string(path)
        .map_err(|e| MycoError::ReadFile { 
            path: path.display().to_string(), 
            source: e 
