...
pub fn pack(package: &PackageDefinition) -> Result<String, MycoError> {
    ...
    zip_directory("./src", &zip_path, ZipOptions {
        ...
        .map_err(|e| MycoError::Internal { message: format!("Failed to create archive: {}", e) })?;

    let raw_toml = std::fs::read_to_string("./myco.toml")
        .map_err(|e| MycoError::ReadFile { 
            path: "./myco.toml".to_string(), 
            source: e 
        })?;
    std::fs::write(toml_path, raw_toml)
...
pub fn bump_version(myco_dir: &PathBuf, myco_toml: &mut MycoToml, matches: &ArgMatches) -> Result<(String, PackageVersion), MycoError> {
    ...
    let mut version = myco_toml.package.as_ref()
        .map(|p| p.version.clone())
        .unwrap_or_else(|| PackageVersion::from_str("0.0.0")
            .map_err(|_| MycoError::PackageVersionDetermination)
            .unwrap_or_else(|_| PackageVersion { major: 0, minor: 0, patch: 0, prerelease: None }));
