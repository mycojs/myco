...
pub fn format_v8_stack_trace_with_source_maps(
    ...
    for i in skip_frames..stack_trace.get_frame_count() {
        if let Some(frame) = stack_trace.get_frame(scope, i) {
            ...
            let mapped_location = map_location_with_source_maps(
                scope,
                &script_name,
                line_number as u32,
            ...
            if let Some((mapped_file, mapped_line, mapped_column)) = mapped_location {
                trace_lines.push(format!(
                    "    at {} ({}:{}:{})",
                    function_name, mapped_file, mapped_line, mapped_column
                ));
            } else {
                trace_lines.push(format!(
                    "    at {} ({}:{}:{})",
    ...
    if let Some(paren_start) = rest.rfind('(') {
        if let Some(paren_end) = rest.rfind(')') {
            if paren_end > paren_start {
                ...
                if let Some((mapped_file, mapped_line, mapped_column)) =
                    parse_and_map_location(scope, location_part)
                {
                    return Some(format!(
                        "    at {} ({}:{}:{})",
                        function_part, mapped_file, mapped_line, mapped_column
                    ));
                }
            }
        }
        ...
        if let Some((mapped_file, mapped_line, mapped_column)) = parse_and_map_location(scope, rest)
        {
            return Some(format!(
                "    at {}:{}:{}",
                mapped_file, mapped_line, mapped_column
            ));
        }
    }
