...
pub enum MycoError {
    ...
    #[error("No workspace.toml found starting from directory '{start_dir}'")]
    WorkspaceNotFound { start_dir: String },
...
impl From<util::UtilError> for MycoError {
    fn from(err: util::UtilError) -> Self {
        match err {
            util::UtilError::FileNotFound { path } => MycoError::FileNotFound { path },
            util::UtilError::FileRead { path, source } => MycoError::ReadFile { path, source },
            util::UtilError::FileWrite { path, source } => MycoError::FileWrite { path, source },
            ...
            util::UtilError::InvalidFilePath { path } => MycoError::Transpilation {
                path,
                message: "Invalid file path".to_string(),
            },
            _ => MycoError::Internal {
                message: err.to_string(),
            },
        }
    }
