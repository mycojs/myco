export default async function(myco: Myco) {
    ...
    const largeWriteToken = await myco.files.requestWrite("/tmp/large_file.txt");
    await largeWriteToken.write(largeContent);
    console.log(`Large file written (${largeContent.length} bytes)`);
    
    const largeReadToken = await myco.files.requestRead("/tmp/large_file.txt");
    ...
    const binaryWriteToken = await myco.files.requestWrite("/tmp/binary_file.bin");
    await binaryWriteToken.write(binaryData);
    console.log(`Binary data written (${binaryData.length} bytes)`);
    
    const binaryReadToken = await myco.files.requestRead("/tmp/binary_file.bin");
    ...
    for (let i = 0; i < concurrentCount; i++) {
        const promise = (async () => {
            const writeToken = await myco.files.requestWrite(`/tmp/concurrent_${i}.txt`);
    ...
    for (let i = 0; i < concurrentCount; i++) {
        const promise = (async () => {
            const readToken = await myco.files.requestRead(`/tmp/concurrent_${i}.txt`);
    ...
    const metadataWriteToken = await myco.files.requestWrite("/tmp/metadata_test.txt");
    await metadataWriteToken.write("Metadata test content");
    console.log("File created");
    
    const metadataReadToken = await myco.files.requestRead("/tmp/metadata_test.txt");
    ...
    for (let i = 0; i < concurrentCount; i++) {
        const cleanupToken = await myco.files.requestWrite(`/tmp/concurrent_${i}.txt`);
