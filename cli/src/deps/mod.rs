...
fn install_from_lockfile(lockfile: &lockfile::LockFile) -> Result<(), MycoError> {
    ...
    std::fs::create_dir_all(".myco")
        .map_err(|e| MycoError::DirectoryCreation { 
            path: ".myco".to_string(), 
            source: e 
        })?;
    
    std::fs::write(".myco/myco.d.ts", MYCO_DTS)
        .map_err(|e| MycoError::FileWrite { 
    ...
    std::fs::write("tsconfig.json", tsconfig_content)
        .map_err(|e| MycoError::FileWrite { 
...
pub fn add(myco_toml: &MycoToml, package: PackageName) -> Result<Vec<DepsChange>, MycoError> {
    if let Some(registries) = myco_toml.registries.clone() {
        ...
        match resolved_package {
            Ok(Some(package)) => {
                let max_version = package.versions.iter().max()
            ...
            Err(e) => {
                Err(MycoError::DependencyResolution { 
                    message: format!("{:?}", e) 
                })
            }
        }
...
pub fn update(myco_toml: &MycoToml, package: Option<PackageName>) -> Result<Vec<DepsChange>, MycoError> {
