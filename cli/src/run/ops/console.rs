...
fn sync_op_print(scope: &mut v8::HandleScope, args: v8::FunctionCallbackArguments, rv: v8::ReturnValue) {
    sync_op(scope, &args, rv, |_scope, input: MessageArg| -> Result<(), MycoError> {
        print!("{}", input.message);
        Ok(())
    });
}

fn sync_op_eprint(scope: &mut v8::HandleScope, args: v8::FunctionCallbackArguments, rv: v8::ReturnValue) {
    sync_op(scope, &args, rv, |_scope, input: MessageArg| -> Result<(), MycoError> {
...
fn sync_op_trace(scope: &mut v8::HandleScope, args: v8::FunctionCallbackArguments, rv: v8::ReturnValue) {
    sync_op(scope, &args, rv, |scope, _input: EmptyArg| -> Result<String, MycoError> {
