export default async function(myco: Myco) {
    ...
    const writeToken = await myco.files.requestWrite("/tmp/test_file.txt");
    await writeToken.write("Hello, Myco Files!");
    console.log("File written successfully");
    
    const readToken = await myco.files.requestRead("/tmp/test_file.txt");
    ...
    const utf8WriteToken = await myco.files.requestWrite("/tmp/utf8_test.txt");
    const utf8Content = "Hello, 世界! 🚀";
    await utf8WriteToken.write(utf8Content);
    
    const utf8ReadToken = await myco.files.requestRead("/tmp/utf8_test.txt");
