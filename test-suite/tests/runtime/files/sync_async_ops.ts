export default async function(myco: Myco) {
    ...
    const asyncWriteToken = await myco.files.requestWrite("/tmp/async_test.txt");
    await asyncWriteToken.write("Async content");
    console.log("Async write completed");
    
    const asyncReadToken = await myco.files.requestRead("/tmp/async_test.txt");
    ...
    const syncWriteToken = await myco.files.requestWrite("/tmp/sync_test.txt");
    syncWriteToken.sync.write("Sync content");
    console.log("Sync write completed");
    
    const syncReadToken = await myco.files.requestRead("/tmp/sync_test.txt");
    ...
    const mixedWriteToken = await myco.files.requestWrite("/tmp/mixed_test.txt");
    await mixedWriteToken.write("Mixed async/sync content");
    
    const mixedReadToken = await myco.files.requestRead("/tmp/mixed_test.txt");
